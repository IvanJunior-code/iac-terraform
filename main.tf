...
provider "digitalocean" {
  token = var.do_token # é a chave que vai fazer a comunicação entre o plugin com a conta do cloud
}


resource "digitalocean_kubernetes_cluster" "k8s_iniciativa" {
  name   = var.k8s_name # nome do cluster na digitalocean
...
resource "local_file" "kube_config" {
    ...
    filename = "kube_config.yaml" # vai colocar o conteudo neste arquivo
}
